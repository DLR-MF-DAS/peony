...
def test_db():
    ...
    session.add(Image(path='/datastore/sentinel/1',
                      geom='POLYGON((0 0, 0.5 0, 0.5 0.5, 0 0.5, 0 0))',
                      date=datetime.date(2019, 1, 1)))
    session.add(Image(path='/datastore/sentinel/2',
                      geom='POLYGON((0.5 0, 1.0 0, 1.0 0.5, 0.5 0.5, 0.5 0))',
                      date=datetime.date(2020, 1, 1)))
    session.add(Image(path='/datastore/sentinel/3',
                      geom='POLYGON((0 0.5, 0.5 0.5, 0.5 1.0, 0 1.0, 0 0.5))',
    ...
    query = session.query(Image).filter(
        Image.geom.ST_Overlaps('POLYGON((0.1 0.1, 0.9 0.1, 0.9 0.4, 0.1 0.4, 0.1 0.1))'))
    for image in query:
        print(image)
