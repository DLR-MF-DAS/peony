...
def pipeline_on_uniform_grid(workdir, pipeline, grid_size, longitude_range=(-180, 180), latitude_range=(-90, 90), n_jobs=1, overlap_percentage=0.0, **kwargs):
    ...
    ny = int(np.ceil((latitude_range[1] - latitude_range[0]) / grid_size)) + 1
...
def grid_progress(logfile, step):
    ...
    with open(os.path.join(workdir, 'info.json'), 'r') as fd:
        info = json.load(fd)
    nx = int(np.ceil((info["longitude_range"][1] - info["longitude_range"][0]) / info["grid_size"])) + 1
    ny = int(np.ceil((info["latitude_range"][1] - info["latitude_range"][0]) / info["grid_size"])) + 1
